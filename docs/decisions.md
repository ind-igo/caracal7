...
- Not done: the `n_cw > 1` split (comptime assert in the verifier, `n_cw() = 1` in Params); the level-2 expected symbols are `k_round_sum` is one thread over 1024 partials; the level-2 expected symbols are one thread per query over `N / 4` symbols.
