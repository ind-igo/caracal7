...
- `backend.mojo`: `Backend` carries only the fields a kernel reads (threadgroup bytes, `mma_k`, `max_terms`, `vec_bytes`, default tile); `BACKEND = SIMD_LANES` is the one value. `tile_mac[B, TM, TN, neg]` is the rank-1 register-tile update (`mma_k = 1`); `gemm.mojo`'s tiled and vec kernels now call it, so the standalone GEMM and the skeleton share the hot op. An MMA backend replaces `tile_mac` and the `Backend` value and nothing above changes.
