...
This page fixes what the specification leaves open: how bytes sit in memory, which kernels exist, and where CPU and GPU meet. The specification (the main prover spec, the statement layer and the polynomial mulmod) is not part of this repository; section numbers below refer to the main spec, and `docs/protocol.md` is the self-contained account of what is built.
...
2. **One buffer type.** All committed data is bytes: one `UInt8` per `F` coordinate. `F2`, `F4`, and `E` values are 2, 4, and `e` consecutive bytes. No struct-of-arrays and no per-field buffer types. Kernels are specialized on the coordinate count, not on a field type. Buffers are not `TileTensor` views: a view per buffer would be a second set of names for offsets the proof format and the transcript fix anyway; the GEMM skeleton's B operand is a `Loader` gather (F bytes as F2, the family descriptor into the LDE, CRT rows with a coset twist) that no affine layout expresses; and the one measurement on this GPU has a `TileTensor` register tile 3x slower than SIMD locals. The question returns inside the skeleton only, behind `Backend`, when an MMA path needs `layout` fragments. The byte vocabulary is one module, `bytes.mojo`: `Base` is the arena pointer every kernel takes, `Buf[W]` a width-typed region of it (one Int64, the width in the type, so an F2 region cannot be passed where E is read), and the host readers of the same layouts. No kernel module defines its own load, store, or integer-field helper. Launchers take the `Arena` and pass its `DeviceBuffer` at enqueue, so the host never manufactures a device pointer; host code reads bytes through `Span[UInt8, _]` and `host_base` is the one place a host Span becomes a `Base`, for the `Hash` methods that run on both sides. Builds are warning-free: `run_tests.sh` passes `--Werror`.
3. **Parameters are comptime.** One `Params` struct carries every knob. Kernels take it as a parameter, so every loop bound and stride is a constant. Exceptions: the GEMM skeleton of section 9 takes `M, N, K` and its operand strides at runtime, because one kernel serves every GEMM-shaped stage; the RS encoder takes the domain (`2^b`, `M`, cosets) and the message length at runtime and the tail kernels take the level sizes and query counts at runtime, because the tail schedule is derived at runtime from `Params` and one kernel serves every level. Tile shapes, reduction cadences, radices and stage strides stay comptime.
...
Milestone 1 profile: `e = 16`, `a1 = 3, m1 = 9` (h1 = 72), `a2 = 5, m2 = 1` (h2 = 32), `L0 = 80,640`, `m_cosets = 1`. This was the reference profile for synthetic columns, not a Keccak grid. Keccak runs the bit layout of statement-layer section 8 on `CLIENT.grid(64, 24 b)` for `b` blocks (`docs/keccak.md`), SHA-256 on `CLIENT.grid(32, 64 b + 1)` (`docs/sha256.md`), Poseidon-M31 on `CLIENT.grid(64, 16 (22 c + 1))` for `c` chunks (`docs/poseidon.md`); the main spec's 72 × 32 and 288 × 128 rows are the table layout, which is not built.

The grid is derived, not named. `Profile` (`params.mojo`) holds the deployment knobs: field, security, tail fold width, clear threshold, leaf size. `Profile.grid(rows_per_chain, chains)` derives a statement's `Params` at compile time: each axis rounded up to the smallest legal `2^a m`, the level-1 code domain by `domain_for` at the profile's `rate_inv` (1/4 for `CLIENT`; the tail levels use the same function at 1/8, `tail_rate_inv`, in place of the spec's 1/32; `docs/soundness.md` says why). A workload declares its row model (rows per chain, and how many chains) and gets its grid; a message-length sweep is a loop inside one binary, a grid sweep is one compile per grid. There is one profile, `CLIENT`; a second appears with a second target (the VM), not with a second grid. Tests run on `CLIENT.grid(72, 32)`, the benches add `CLIENT.grid(288, 128)`. The spec's throughput proxy, `CLIENT.grid(2016, 576)`, fails `check()` until the codeword split exists: its 290,304 symbols per column exceed the largest domain at rate 1/4. Even then the allocate-once arena is 5.3 GB at 91 W columns and 11.7 GB at 357 on that grid; arena region reuse is the next item there.
...
| `lde` | coeff → lde | `dft_axis`, three radix stages per axis (four when the odd part splits, 63 = 7 x 9) | forward DFT onto `G` with the twist inside the plan tables |
...
| `small grid` | z2, chain ends, wiring lines, alpha → q3 | `dft_axis` plans over one line of 8 F2 lanes for the inverse DFTs (up to six lines per term), the coset evaluation and the G2 evaluation, the coset twists folded into the stage tables; one thread per coset point for the products and the division | R2 on the coset gamma2 G2, Q3 = R2 / (X2^h2 - 1) pointwise there, then to coefficients and to values on G2 (spec 7.4) |
| `residual` | lde, tables → residual | `k_residual`, one thread per point | the fused pass of statement-layer 5: each thread gathers its point's family rows from the LDE and accumulates the 8 kappa lanes in registers (the GEMM skeleton lost at M = 8); milestone 1 runs it on synthetic families |
| `quotient` | residual → trace of A, B, Q2 | GEMM skeleton for the coset steps, `dft_axis` for the full-length transforms | `Q1` on the coset from `R` over `G1`, inverse DFTs to the `A`, `B`, `Q2` coefficients, forward DFTs to their values on `H`; the coordinate columns then take the witness encoder path |

Milestone 1 ended with the W and Q trees, the residual and quotient on synthetic families, openings at P points, the tail, and the verifier, as the spec's build order says. Milestone 2 adds the Z tree (`accumulate`, committed through the same encoder and Merkle path, alpha after it), the boundary opening points, and the small grid with Q3. Milestone 3 adds the Herder lookup (`docs/lookup.md`). Public columns and restrictions are `docs/public-columns.md`. The statement builder above the IR is `relations/statement.mojo`.
...
- `hash.mojo`: the `Hash` trait (leaf, node, absorb, squeeze, and absorb's parallel form chunk/merge; `DIGEST`, `BLOCK`, `CHUNK`). Merkle, transcript, prover, and verifier take `H: Hash` as a comptime parameter next to `Params`; Blake3 is the first implementation, and a second one (for recursion) changes nothing above the trait.
- `transcript.mojo`: device-resident. A small buffer holds the running Blake3 state; one kernel absorbs a message (tree root, openings, clear values, sumcheck messages) with the domain separator of 9.4, one thread per 1 KB chunk of the tree hash in one block, one kernel squeezes challenges into a device buffer: E elements as e bytes, positions as uniform integers below L. Every kernel that needs a challenge reads it from that buffer.
- `verifier.mojo`: host program, separate from the prover. The seven steps of statement-layer section 6 reduced to the Ligerito checks; the tail runs in tensor form (`pcs/tensor.mojo`: every batched functional is a list of digit products per odd index, folded per level at the sumcheck challenges, summed against the clear vector at the end), so the verifier never holds a vector of length N.
...
src/
  ...
  relations/        the Caracal IR and its arguments; __init__ re-exports the prover, verifier, and proof surface
    ...
    sort.mojo         the lookup's sorted copy: counting sort over the advice index (lookup.md)
...
**Arithmetic on bytes.** F values are 7 bits. A product is under 2^14, so a `UInt16` accumulator holds four products and an `Int32` holds thousands before a reduction. Reduce lazily: accumulate wide, reduce once per tile with `(x & 127) + (x >> 7)` repeated until the value is below 254 (three rounds for a 32-bit accumulator of up to 2^20, two for 16 bits), then map 127 to 0; no division. Verified on the M1 GPU with a 64 × 64 × 64 byte GEMM against a scalar reference. F2 and F4 products are 4 and 16 base MACs on the same accumulators. The GEMM skeleton and the gather keep integer lanes. The RS stage kernels use fp32 lanes (`field.mojo` fp_ helpers): values stay exact integers below 2^24, a reduction is `x - 127 round(x / 127)` in four ops per lane (the integer fold is thirteen), twiddles load centered in [-63, 63] so products stay small, and a value is canonicalized once at the store. The ALU-bound stages halved.
...
- The tower constants `c_2`, `c_3`, `c_4` for `e = 16`; `G5 = i + j` and the fifth root of unity for `e = 20` (`field.mojo`).
