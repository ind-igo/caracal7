"""The secp256k1 ECDSA circuit: op counts on the 144 x 576 grid, the live walk emitting the fixed circuit op for op,
...
from workloads.ecdsa_k1 import secp256k1, EcdsaK1, walk
...
def test_circuit_shape_and_host_walk() raises:
    ...
    var sig = _signature()
    var w = walk(secp256k1(), sig[0], sig[1], sig[2], sig[3], True)
    ...
    var bad = walk(secp256k1(), sig[0], sig[1], sig[2] + Big(1), sig[3], True)
    with assert_raises(contains="does not hold"):
        _ = circuit_values(bad.inputs, bad.ops, bad.hints)
    with assert_raises(contains="(0, n)"):
        _ = walk(secp256k1(), sig[0], Big(), sig[2], sig[3], True)


def test_prover_round_trip() raises:
    var sig = _signature()
    var w = EcdsaK1(sig[0].copy(), sig[1].copy(), sig[2].copy(), sig[3].copy())
    var claim = w.public_inputs[p]()
    var ctx = DeviceContext()
    var proof = prove_workload[p, Blake3, EcdsaK1](ctx, w)
    print("ecdsa proof bytes:", len(proof))
    var again = proof.copy()
    assert_true(verify_workload[p, Blake3, EcdsaK1](proof^, w, claim, profile=True))
    var tampered = claim.copy()
    tampered[64] ^= 1
    var accepted: Bool
    try:
        accepted = verify_workload[p, Blake3, EcdsaK1](again^, w, tampered)
    except:
        accepted = False
