...
def test_openings_and_fold() raises:
    ...
    var y = _dl(ctx, prover, L.open.fold_y, N * p.e)
    var code_w = _dl(ctx, prover, L.w.enc.code, p.L() * SYNTHETIC_COLUMNS * 4)
    var code_z = _dl(ctx, prover, L.z.enc.code, p.L() * shape.columns_z * 4)
    var code_q = _dl(ctx, prover, L.q.enc.code, p.L() * shape.columns_q * 4)
