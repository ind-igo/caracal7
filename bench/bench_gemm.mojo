...
from caracal7.backend import BACKEND, F4_TILE, Tile, Strided, Strided4, launch_gemm_f2, launch_gemm_f4, strided
...
def main() raises:
    ...
    run[Tile(BM=64, BN=64, BK=8, TM=4, TN=4)](ctx, arena, a, b, c, "64/64/8 4x4")
    run[Tile(BM=64, BN=64, BK=16, TM=4, TN=8)](ctx, arena, a, b, c, "64/64/16 4x8")
    run4[F4_TILE](ctx, arena, a, b, c, "F4 default 32/32/8 2x2")
    run4[Tile(BM=64, BN=64, BK=8, TM=4, TN=4)](ctx, arena, a, b, c, "F4 64/64/8 4x4")
    run4[Tile(BM=32, BN=64, BK=8, TM=2, TN=4)](ctx, arena, a, b, c, "F4 32/64/8 2x4")
    run4[Tile(BM=32, BN=32, BK=16, TM=2, TN=2)](ctx, arena, a, b, c, "F4 32/32/16 2x2")
