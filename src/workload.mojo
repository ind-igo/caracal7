...
def prove_workload[p: Params, H: Hash, W: Workload](ctx: DeviceContext, w: W) raises -> List[UInt8]:
    ...
    return prove_prepared[p, H, W](ctx, prover, w, layout, w.public_inputs[p]())


def prove_prepared[p: Params, H: Hash, W: Workload](ctx: DeviceContext, mut prover: Prover[p, H], w: W, layout: Layout,
                                                     public: List[UInt8]) raises -> List[UInt8]:
    """Everything that depends on the inputs, on a constructed prover: trace, advice, public data, loads, prove."""
...
struct Session[p: Params, H: Hash, W: Workload & Movable & Deinitable]:
    """A workload with its prover built and settled. The csp-benchmarks Rust track times `prove` on a
    prepared context, so the constructor is the prepare (compile, prover construction, tables, the arena
    fill) and `prove` is every step that depends on the inputs. `public` empty means `w.public_inputs`."""
    var ctx: DeviceContext
    var w: Self.W
    var layout: Layout
    var prover: Prover[Self.p, Self.H]
    var public: List[UInt8]

    def __init__(out self, ctx: DeviceContext, var w: Self.W, var public: List[UInt8] = List[UInt8]()) raises:
        var c = w.statement[Self.p]().compile[Self.p]()
        self.ctx = ctx
        self.layout = c.layout.copy()
        self.prover = Prover[Self.p, Self.H](ctx, c^)
        self.public = public^ if len(public) > 0 else w.public_inputs[Self.p]()
        self.w = w^
        ctx.synchronize()

    def prove(mut self) raises -> List[UInt8]:
        return prove_prepared[Self.p, Self.H, Self.W](self.ctx, self.prover, self.w, self.layout, self.public)

    def verify(self, var proof: List[UInt8]) raises -> Bool:
        return verify_workload[Self.p, Self.H, Self.W](proof^, self.w, self.public)

    def cells(self) -> Int:
        """Committed witness cells: the circuit size the benchmark reports."""
        return self.prover.shape.columns_w * Self.p.N()

    def preprocessing_bytes(self) -> Int:
        """The per-grid tables built in the constructor (uploaded to the arena before any input is seen)."""
        var n = self.prover.layout.tables.bytes
        for i in range(len(self.prover.layout.tail)):
            n += self.prover.layout.tail[i].rs.bytes
        return n
